...
function Afiseaza_programri()
{
		global $conn;
		$id = $_SESSION['id']; 
		$sql = "select id,(select nume from furnizori where id=id_furnizor)\"Furnizor\",(select tip from servicii where id=id_serviciu)\"Serviciu\",to_char(data_programare,'dd-mm-yy')\"Data Programarii\",to_char(ora_programare,'HH24:MI')\"Ora Programarii\",atendance\"Atendance\" from programari where id_client = ".$id."order by\"Data Programarii\",\"Ora Programarii\"";
...
$adaug_ora = $_POST['adaug_ora'];
	    $sql='BEGIN add_programare(:id,:id_f,:id_s,:data,:ora); commit; END;';
		$stmt = oci_parse($conn,$sql);
		oci_bind_by_name($stmt,':id',$id,32);
		oci_bind_by_name($stmt,':id_f',$adaug_idm,32);
		oci_bind_by_name($stmt,':id_s',$adaug_ids,32);
		oci_bind_by_name($stmt,':data',$adaug_data,32);
		oci_bind_by_name($stmt,':ora',$adaug_ora,32);
		oci_execute($stmt);
}
...
function Sterge_programare()
{
		...
		$sql = 'BEGIN delete_programare(:id); END;';
		$stmt = oci_parse($conn,$sql);
		oci_bind_by_name($stmt,':id',$sterg_idp);
		oci_execute($stmt);
}
...
<h2>Sterge programare </h2>
 ...
 <!-- <input type="user" name="sterg_idm" placeholder="Id magazin"><br><br>
  ...
  <input type="submit" name="stergeprogramare" value="Stergere">
</form>
